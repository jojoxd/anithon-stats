...
var syncUserCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		seriesService := domain.NewSeriesService(database.SeriesRepository(), translationService, logger)
		...
		if err := syncService.SyncUserImmediate(cmd.Context(), args[0], tx); err != nil {
